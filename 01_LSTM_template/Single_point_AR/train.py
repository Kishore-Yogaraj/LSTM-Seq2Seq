...
def split_sequence(sequence, n_step):
    ...
    for j in range(len(sequence)):
        ...
        if end_ix > len(sequence) - 1:
            return X, y
        seq_x, seq_y = sequence[j:end_ix], sequence[end_ix]
        X.append(seq_x)
        y.append(seq_y)
    return X, y
...
def process_configuration(first_layer_dir, filtered_raw_seq, kp, ps, step, batch):
    ...
    X_train, y_train = split_sequence(train_seq, step)
    ...
    model.add(Bidirectional(LSTM(units=25, activation='relu', return_sequences=True),
                            input_shape=(step, n_features)))
    model.add(Bidirectional(LSTM(units=25, activation='relu', return_sequences=True)))
    model.add(Bidirectional(LSTM(units=25, activation='relu', return_sequences=False)))
...
#     # early_stopping = EarlyStopping(monitor='loss', min_delta=1, patience=patience, verbose=1, mode='min', restore_best_weights=True)
...
#     # === Save model using SavedModel format ===
#     saved_model_path = os.path.join(second_layer_dir, "saved_model")
#     model.save(saved_model_path)
#     print(f"Model saved in TensorFlow SavedModel format at: {saved_model_path}")

#     # start_time = time.time()
#     # history = model.fit(X_train, y_train, epochs=n_epochs, verbose=2, batch_size=batch, callbacks=[early_stopping])
#     # end_time = time.time()

#     # # Save the model after training
#     # model_path = os.path.join(second_layer_dir, "trained_model.h5")
#     # model.save(model_path)
#     # print(f"Model saved to {model_path}")

#     # predictions = []
#     # x_input = train_seq[-step:]
#     # x_input = np.array(x_input).reshape((1, step, n_features))
